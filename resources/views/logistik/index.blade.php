    ...
    <div class="conatiner-fluid content-inner py-3">
        <div>
            <div class="row">
                ...
                {{-- SECTION Rincian Data --}}
                <div class="col-lg-7">
                    {{-- Judul --}}
                    <h3><strong>Informasi Pengiriman Kelapa Sawit</strong></h3>
                    {{-- END Judul --}}
                    {{-- FIXME: masih menggunakan akses array bukan collection --}}
                    {{-- Rincian --}}
                    <div>
                        {{-- Sopir --}}
                        <x-tabel.two-lines-rincian :name="__('Sopir:')" :isi="$logistik['sopir']" />
                        {{-- END Sopir --}}
                        {{-- Plat --}}
                        <x-tabel.two-lines-rincian :name="__('Plat Nomor:')" :isi="$logistik['plat']" />
                        {{-- END Plat --}}
                        {{-- Kebun --}}
                        <x-tabel.two-lines-rincian :name="__('Asal Kebun:')" :isi="$logistik['kebun']" />
                        {{-- END Kebun --}}
                        {{-- Pabrik --}}
                        <x-tabel.two-lines-rincian :name="__('Tujuan Pabrik:')" :isi="$logistik['pabrik']" />
                        {{-- END Pabrik --}}
                        {{-- Bruto --}}
                        {{-- FIXME: di desain sebutannya total berat, padahal ada TBS bawah --}}
                        <x-tabel.two-lines-rincian :name="__('Berat Kotor:')" :isi="$logistik['brutto'] . ' kg'" />
                        {{-- END Bruto --}}
                        {{-- Netto --}}
                        {{-- FIXME: di desain sebutannya total TBS, padahal ada total berat atas --}}
                        <x-tabel.two-lines-rincian :name="__('Berat Bersih:')" :isi="$logistik['netto'] . ' kg'" />
                        {{-- END Netto --}}
                        {{-- Harga --}}
                        {{-- TODO: aktifkan intl php untuk gunakan Currency Number --}}
                        <x-tabel.two-lines-rincian :name="__('Harga TBS/kg:')" :isi="'Rp.' . $logistik['harga']" />
                        {{-- END Harga --}}
                    </div>
                    {{-- END Rincian --}}
                    {{-- Map --}}
                    <div>
                        <H1 style="position: absolute"><strong>GMAPS</strong></H1>
                        <img class="rounded" src="https://picsum.photos/800" alt=""
                            style="max-width:100%; height:auto">
                    </div>

                    {{-- END Map --}}
                </div>
                {{-- END SECTION Rincian Data --}}
            </div>
        </div>
    </div>
